...
# include <windows.h>
static __inline__ int
SDL_atomic_int_xchg_add(volatile LONG* atomic, LONG value)
{
  return InterlockedExchangeAdd(atomic, value);
}

static __inline__ void
SDL_atomic_int_add(volatile LONG* atomic, LONG value)
{
...
static __inline__ SDL_bool
SDL_atomic_ptr_cmp_xchg(volatile PVOID* atomic, PVOID oldvalue, PVOID newvalue)
{
  return (SDL_bool)(InterlockedCompareExchangePointer(atomic, newvalue, oldvalue) == oldvalue);
}
# else /* WINVER <= 0x0400 */
#  if (SIZEOF_VOIDP != 4)
